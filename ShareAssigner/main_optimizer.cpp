...
int main(int argc, char *argv[]) {
  ...
  if (inputFile.ends_with(".txt")) {
    ...
    inputTermNetwork =
        delayedresharing::GeneralizedTermNetwork::fromFile(inputFile);
  } else if (inputFile.ends_with(".bristol")) {
    std::cout << "Bristol file format found." << "\n";
    inputTermNetwork =
        delayedresharing::GeneralizedTermNetwork::fromBristol(inputFile);
  }

  delayedresharing::CostModel *costModel =
      new delayedresharing::AnalyticalThreePCCostModel();

  delayedresharing::CommunicationMetric *metric =
      new delayedresharing::CommunicationMetric(costModel);

  delayedresharing::Partition *partitioningPass =
      new delayedresharing::Partition();
  partitioningPass->metric = metric;
  partitioningPass->k = 1;

  delayedresharing::GeneralizedTermNetwork *networkFresh =
      partitioningPass->applyFresh(inputTermNetwork);

  if (costModelString == "offline") {
    std::cout << "Offline\n";
    delayedresharing::OfflineBoundaryShareAssignment
        *offlineBoundaryShareAssignment =
            new delayedresharing::OfflineBoundaryShareAssignment();
    offlineBoundaryShareAssignment->apply(inputTermNetwork);
    return 0;
  }

  delayedresharing::AstraShareAssignment *astraShareAssignment =
      new delayedresharing::AstraShareAssignment();
  astraShareAssignment->apply(networkFresh);

  networkFresh->toFile(outputFileBaseline);
  std::cout << "result: " << metric->compute(networkFresh) << "\n";

  delayedresharing::BoundaryShareAssignment *boundaryShareAssignment =
      new delayedresharing::BoundaryShareAssignment();
  ...
  if (costModelString == "3shamir") {
    ...
    boundaryShareAssignment->costModel =
        delayedresharing::BoundaryShareAssignment::Model::SHAMIR;
  ...
  if (costModelString == "replicated") {
    std::cout << "Replicated\n";
    boundaryShareAssignment->costModel =
        delayedresharing::BoundaryShareAssignment::Model::REPLICATED;
  }
  if (costModelString == "masked") {
    std::cout << "Masked\n";
    boundaryShareAssignment->costModel =
        delayedresharing::BoundaryShareAssignment::Model::MASKED;
  }
